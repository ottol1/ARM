...
int grip_rot = 0, fs, send_freq = 20, grip_command, pos_last = 0;  // t_prev, t_now;
bool grip_present = false;
...
void grip_init() {
  grip_command = 600;  // 0 - 1023, 0N - 1N
  while (fs < (grip_command - e_fs)) {
  ...
  grip_rot = 0;
  pos_last = dxl1.getPresentPosition(6);
  // after gripper has closed, open it, and keep track of rotations
  grip_open();
}

void grip_write() {
  grip_command = goalPos[5];  // 0 - 1023, 0N - 1N
  fs = analogRead(FS_PIN);    // 0 - 1023, 0V - 3.3V, 0N - 1N
  ...
  // if gripper is closed too much, open slowely a bit
    ...
    dxl1.writeControlTableItem(MOVING_SPEED, 6, 0);  // stop the gripper
  }
  pos_last = pos_current;
}
