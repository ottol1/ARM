...
model.set_classes(["hand"]) # Only include this line if using YOLO World. Select what to put boxes around
...
while True:
    ...
    for r in results:
        boxes = r.boxes
        for box in boxes:
            x1, y1, x2, y2 = box.xyxy[0].to('cpu').detach().numpy().copy()  # get box coordinates in (top, left, bottom, right) format
            c = box.cls # labeling the depth map boxes with their class
            ...
            #cv2.rectangle(depth_colormap, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), # if you want to see the depth map boxes
            #              thickness = 2, lineType=cv2.LINE_4)
            #cv2.putText(depth_colormap, text = model.names[int(c)], org=(int(x1), int(y1)), # put the labels on the depth map boxes
            #            fontFace = cv2.FONT_HERSHEY_SIMPLEX, fontScale = 0.7, color = (0, 0, 255),
            #            thickness = 2, lineType=cv2.LINE_4)
            cv2.putText(color_image, text = dist2, org=(int(x1), int(y1)), # label the box with the dist2 value
                        fontFace = cv2.FONT_HERSHEY_SIMPLEX, fontScale = 0.7, color = (0, 0, 255),
                        thickness = 2, lineType=cv2.LINE_4)
            cv2.rectangle(color_image, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), # create bounding box
                          thickness = 2, lineType=cv2.LINE_4)
